...
public class ScriptComponent : Component, IDisposable {
    ...
    protected ScriptSystem ScriptSystem => World.GetSystem<ScriptSystem>();
    ...
    public Task DelayFrame(int frameCount) => Dispatcher.Delay<TickUpdateEvent>(frameCount).AsTask();

    public Task<T> DelayTime<T>(TimeSpan duration) where T : ITickEvent => Dispatcher.Delay<T>(duration).AsTask();
        
    public Task DelayTime(TimeSpan duration) => Dispatcher.Delay<TickUpdateEvent>(duration).AsTask();

    public Task<T> UntilCondition<T>(Func<T, bool> condition) where T : ITickEvent => Dispatcher.Until(condition, 0).AsTask();
        
    public Task UntilCondition(Func<TickUpdateEvent, bool> condition) => Dispatcher.Until(condition, 0).AsTask();
...
//
//     protected ScriptSystem Scripts => Worlds.GetChild<ScriptSystem>();
//
//     protected ContentManager Contents => Worlds.Services.Get<ContentManager>();
//
...
//     protected Logger Logger => logger ?? (logger = Worlds.Services.Get<LogManager>().Get(this.GetType().FullName));
//
