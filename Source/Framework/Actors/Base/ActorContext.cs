...
public readonly struct ActorContext : IDisposable {
    ...
    public ActorContext(ActorNode node, ActorRef sender, CancellationToken cancellation) {
        Node = node;
        Sender = sender;
    ...
    public ActorContext(ActorNode node, ActorRef sender, TaskCompletionSource<IResponse> completion) {
        Node = node;
        Sender = sender;
